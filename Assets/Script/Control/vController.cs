...
public class vController : MonoBehaviour {
	...
	private static vController[] joysticks;					// A static collection of all joysticks
	...
	public bool Scale;											// Current tap count
	public float prefix;
	...
	void Update() {
		if (!enumeratedJoysticks) {
			...
			joysticks = (vController[])FindObjectsOfType(typeof(vController));
		...
		else
		{
			for(int i = 0; i < count; i++) {
				...
				if (shouldLatchFinger && (lastFingerId == -1 || lastFingerId != touch.fingerId )) {
					...
					foreach (vController j in joysticks) {
						if (j != this)
